...
export default function CandidateLayout({
  ...
  const notificationsContent = candidateNotificationsContentFor(language);
  const { unreadCount: unread } = useUnreadNotifications();
  ...
  return (
    <div className="min-h-screen bg-surface lg:flex">
      ...
      <aside className="hidden lg:sticky lg:top-0 lg:flex lg:h-screen lg:w-64 lg:shrink-0 lg:flex-col lg:border-r lg:border-outline-variant lg:bg-surface-container-lowest">
        ...
        <Link
          href="/notifications"
          className="mx-4 mb-3 flex items-center justify-between rounded-xl border border-outline-variant px-3 py-2 text-sm font-bold text-primary"
        >
          <span>{notificationsContent.title}</span>
          {unread > 0 && (
            <span className="rounded-full bg-error px-2 py-0.5 text-xs text-onError">
              {unread}
            </span>
          )}
        </Link>
        ...
        <nav
          ...
          aria-label={t("nav_candidate_aria_label")}
        >
        ...
        <nav className="fixed inset-x-0 bottom-0 z-30 mx-auto flex max-w-md items-center justify-around border-t border-surface-container-high bg-surface-container-lowest/95 px-3 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 shadow-subtle backdrop-blur-md lg:hidden">
          {TABS.map(({ href, label, icon }) => {
            ...
            return (
              ...
              >
                <div
                  className={`flex items-center justify-center rounded-full px-3 py-1.5 transition-colors ${isActive ? "bg-surface-container-low text-primary" : ""}`}
                >
                  <span
                    className={`material-symbols-outlined ${isActive ? "fill text-primary" : "text-onSurface-variant"}`}
                    style={{ fontSize: 24 }}
                  >
                    {icon}
                  </span>
