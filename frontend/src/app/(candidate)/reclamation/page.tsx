...
import {
  candidateReclamationContentFor,
  categoryLabelFor,
} from "@/lib/candidateReclamationContent";
...
export default function ReclamationPage() {
  ...
  // Replies now produce notifications. Refresh when the user returns to the
  // tab instead of polling the API every three seconds.
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const entry = voiceBlob
        ...
        : await complaintsRepository.submitText(
            category,
            message.trim(),
            token,
          );
      ...
      setVoiceUrl(null);
      setTicketRef(
        `AMU-${new Date().getFullYear()}-${String(entry.id).padStart(4, "0")}`,
      );
    } catch (cause) {
  ...
  return (
    <div className="min-h-screen bg-surface pb-24">
      <header className="sticky top-0 z-10 flex h-16 items-center border-b border-surface-container bg-surface px-2.5 lg:px-4">
        <Link
          href="/dashboard"
          className="mr-4 text-primary-dark transition-transform active:scale-95"
        >
      ...
      <main className="mx-auto max-w-md space-y-8 px-6 pt-8 lg:max-w-5xl lg:px-10 lg:pt-10">
        <div className="flex flex-col items-center lg:items-start lg:text-left">
          <div className="mb-4 flex h-24 w-24 items-center justify-center rounded-full bg-primary-container shadow-sm">
            <span
              className="material-symbols-outlined fill text-on-primary"
              style={{ fontSize: 44 }}
            >
              report_problem
            </span>
          </div>
          <h2 className="text-center text-2xl font-bold text-primary-dark lg:text-left">
            {content.intro.title}
          </h2>
          <p className="mt-2 text-center text-sm text-onSurface-variant lg:text-left">
            {content.intro.body}
          </p>
        </div>
        ...
        <div className="space-y-8 lg:grid lg:grid-cols-[1fr_340px] lg:items-start lg:gap-8 lg:space-y-0">
            ...
            <section className="space-y-6">
              <div className="space-y-2">
                ...
                >
                  <option value="" disabled>
                    {content.form.subjectPlaceholder}
                  </option>
                  {CATEGORIES.map((c) => (
                    <option key={c} value={c}>
                      {categoryLabelFor(content, c)}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex items-center gap-4 py-2">
                ...
                <span className="text-xs font-bold text-outline">
                  {content.form.orDivider}
                </span>
                <div className="h-px flex-1 bg-outline-variant" />
              </div>

              <div className="space-y-4 rounded-xl bg-surface-container-low p-4">
                <p className="text-center text-sm font-semibold text-onSurface">
                  {content.form.voiceTitle}
                </p>
                <AudioRecorder
          ...
          <section className="space-y-3 pb-6">
            <h2 className="text-sm font-bold text-primary-dark">
              {content.history.titlePrefix} ({entries.length})
            </h2>
            <div className="space-y-2.5">
              {entries.map((entry) => {
                ...
                return (
                  <div
                    key={entry.id}
                    className="rounded-xl border border-outline-variant bg-surface-container-lowest p-3.5 shadow-soft"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-sm font-bold text-onSurface">
                        {subject ??
                          (entry.type === "voice"
                            ? content.history.voiceFallback
                            : content.history.textFallback)}
                      </span>
                      <span
                    ...
                    {entry.admin_response && (
                      <p className="mt-2 rounded-lg bg-surface-container p-2 text-xs text-onSurface">
                        <span className="font-bold">
                          {content.history.responsePrefix}{" "}
                        </span>
                        {entry.admin_response}
                      </p>
                    )}
        ...
        <div className="fixed inset-0 z-[60] flex flex-col items-center bg-primary-dark/40 px-6 backdrop-blur-sm">
          ...
          <div className="w-full max-w-sm rounded-3xl bg-surface-container-lowest p-8 text-center shadow-2xl">
            ...
            <p className="mb-6 text-sm text-onSurface-variant">
              {content.successModal.body}
            </p>
            <div className="inline-block rounded-lg border border-outline-variant bg-surface-container p-3">
              <span className="block text-xs font-bold uppercase tracking-wider text-onSurface-variant">
                {content.successModal.ticketLabel}
              </span>
            ...
            <Button
              ...
              className="mt-8 bg-primary-dark"
            >
