...
export default function SwipeMatchingsPage() {
  ...
  const handleDecision = (liked: boolean) => {
    if (liked && candidate) {
      ...
      setToast(t('employer:matchingsSwipe.interestExpressed'));
      ...
      <div className="mt-6">
        {!candidate ? (
          <div className="rounded-2xl bg-surface-container p-10 text-center">
            <p className="text-sm font-semibold text-onSurface-variant">{t('employer:matchingsSwipe.empty')}</p>
            <button
              type="button"
              onClick={() => setIndex(0)}
              className="mt-4 rounded-xl bg-navy-900 px-4 py-2 text-xs font-bold text-white"
            >
              {t('employer:matchingsSwipe.restart')}
            </button>
          </div>
        ) : (
          <div className="relative rounded-3xl border border-gray-100 bg-white p-6 shadow-floating">
            ...
            <p className="mt-1 flex items-center justify-center gap-1 text-xs text-gray-500">
              <MapPin size={11} /> {t('employer:matchingsSwipe.cityYears', { city: candidate.city, years: candidate.yearsExperience })}
            </p>
            <div className="mt-4 flex justify-center gap-2">
              ...
              <span className="rounded-full bg-amber-100 px-2.5 py-1 text-[11px] font-bold text-amber-700">
                <span dir="ltr">{candidate.matchScore}%</span> {t('employer:matchingsSwipe.matchLabel')}
              </span>
            </div>
            ...
            <div className="mt-8 flex justify-center gap-6">
              <button
                ...
                aria-label={t('employer:matchingsSwipe.skip')}
              >
                <X size={22} />
              </button>
              <button
                ...
                aria-label={t('employer:matchingsSwipe.expressInterest')}
              >
                <Heart size={22} />
              </button>
            </div>
