...
export interface AuthRepository {
  requestOtp(phone: string, referralToken?: string): Promise<OtpRequestResponse>;
...
const httpAuth: AuthRepository = {
  ...
  logout: (token) => apiPost<void>('/auth/logout', {}, token).then(() => undefined),
};

const mockAuth: AuthRepository = {
  requestOtp: (phone) => {
    if (phone.replace(/\D/g, '').length < 9) {
      return fakeFailure(new ApiError(422, 'Numéro invalide', { reason: 'invalid' }));
    }
  ...
  verifyOtp: (phone, code) => {
    if (code !== MOCK_OTP_CODE) {
      return fakeFailure(new ApiError(422, 'Code incorrect', { reason: 'invalid' }));
    }
...
export const authRepository: AuthRepository = process.env.NEXT_PUBLIC_USE_MOCKS === '1' ? mockAuth : httpAuth;
