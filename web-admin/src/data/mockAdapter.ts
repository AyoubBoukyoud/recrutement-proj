...
const routes: Array<[string, string, Handler]> = [
  ...
  [
    ...
    (ctx) => {
      ...
      let rows = state.adminCandidates
  ...
  [
    ...
    (ctx) => {
      ...
      const row = state.adminCandidates.find((c) => c.id === id)
  ...
  [
    ...
    (ctx) => {
      const status = ctx.query.get('status')
      const rows = status ? state.complaints.filter((c) => c.status === status) : state.complaints
      return paginate(rows, pageOf(ctx))
    },
  ],
  [
    ...
    '/admin/complaints/:id',
    (ctx) => {
      const complaint = state.complaints.find((c) => c.id === numberParam(ctx))
  ...
  [
    ...
    (ctx) => {
      ...
      const rows = includeInactive ? state.tasks : state.tasks.filter((t) => t.is_active)
      return paginate(rows, pageOf(ctx))
    },
  ],
  [
  ...
  [
    ...
    '/admin/tasks/:id',
    (ctx) => {
      const task = state.tasks.find((t) => t.id === numberParam(ctx))
  ...
  ['GET', '/admin/users', (ctx) => paginate(state.users, pageOf(ctx))],
  [
    'PATCH',
    '/admin/users/:id/roles',
    (ctx) => {
      const user = state.users.find((u) => u.id === numberParam(ctx))
  ...
  ['GET', '/admin/referrals', (ctx) => paginate(state.payouts, pageOf(ctx))],
  [
    'PATCH',
    '/admin/referrals/:id',
    (ctx) => {
      const payout = state.payouts.find((p) => p.id === numberParam(ctx))
  ...
  [
    ...
    (ctx) => {
      let rows = state.candidates.filter((c) => c.submitted)
  ...
  [
    ...
    (ctx) => {
      const id = numberParam(ctx)
      state.shortlist = state.shortlist.filter((r) => r.candidate_profile_id !== id)
  ...
  [
    ...
    (ctx) => {
      ...
      const existing = state.shortlist.find((r) => r.candidate_profile_id === id)
      if (existing) {
  ...
  ['GET', '/recruiter/shortlist', (ctx) => paginate(state.shortlist, pageOf(ctx))],
  [
    'GET',
    '/recruiter/shortlist/export',
    () => {
      const header = 'nom,metier,experience,disponibilite\n'
      const body = state.shortlist
        .map((r) =>
  ...
  /* --- Agent commercial --- */
  ['GET', '/referrals/agent', () => state.agent],
